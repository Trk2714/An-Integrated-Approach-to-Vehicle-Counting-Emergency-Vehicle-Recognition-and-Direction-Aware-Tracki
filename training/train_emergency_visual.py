...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLOv8 emergency visual detector.")
...
def main() -> None:
    ...
    data_yaml = PROJECT_ROOT / "data" / "processed" / "emergency_visual_yolo.yaml"
    if not data_yaml.exists():
        raise FileNotFoundError(f"Emergency visual dataset yaml not found: {data_yaml}")
    ...
    train_results = model.train(
        ...
        name="emergency_visual_detector",
        exist_ok=True,
        pretrained=True,
        patience=1000,
        verbose=True,
    ...
    out_best = PROJECT_ROOT / "models" / "emergency_visual_best.pt"
    out_last = PROJECT_ROOT / "models" / "emergency_visual_last.pt"
    ...
    write_json(PROJECT_ROOT / "models" / "emergency_visual_metrics.json", metrics_payload)
    print("Emergency visual detector training complete.")
